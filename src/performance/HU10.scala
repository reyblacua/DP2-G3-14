...
class HU10 extends Simulation {
	...
	object CannotAddAppointmentWithoutPayingPreviousOnes {
		val cannotAddAppointmentWithoutPayingPreviousOnes = exec(http("Cannot add appointment")
			.get("/hairdressers/1/appointments/new")
			.headers(headers_0)
		).pause(10)
	}
	...
	val scn2 = scenario("Cannot create appointment without paying previous").exec(
		Home.home,
		Login2.login2,
		ShowHairdressers.showHairdressers,
		ShowHairdresser.showHairdresser,
		CannotAddAppointmentWithoutPayingPreviousOnes.cannotAddAppointmentWithoutPayingPreviousOnes
	)
		
		
	setUp(scn1.inject(rampUsers(5000) during (100 seconds)),
