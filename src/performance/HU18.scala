...
class HU18 extends Simulation {
	...
	object LoginOwner10 {
        val loginOwner10 = exec(http("LoginOwner10")
            .get("/login")
            .headers(headers_0)
            .check(css("input[name=_csrf]","value").saveAs("stoken"))
        ).pause(10)
        .exec(http("LoggedOwner10")
            .post("/login")
            .headers(headers_2)
	...
	object NewInscription10 {
		val newInscription10 = exec(http("NewInscription10")
			.get("/courses/1/inscription/new")
			.headers(headers_2)
			.check(css("input[name=_csrf]","value").saveAs("stoken")))
		.pause(10)
	...
	val scn2 = scenario("scn2").exec(Home.home,
									  LoginOwner10.loginOwner10,
									  ShowCourses.showCourses,
									  ShowCourse.showCourse,
									  NewInscription10.newInscription10)


	setUp(scn1.inject(rampUsers(6500) during (100 seconds)),
			scn2.inject(rampUsers(6500) during (100 seconds))
