...
class HU11 extends Simulation {
	...
	object CannotAddAppointmentForSameDay {
		val cannotAddAppointmentForSameDay = exec(http("Add new appointment")
			.get("/hairdressers/1/appointments/new")
			.headers(headers_0)
    	.check(css("input[name=_csrf]", "value").saveAs("stoken"))
		).pause(30)
		.exec(http("Appointment created")
			...
			.formParam("name", "new appointment")
			.formParam("description", "a brand new appointment")
			.formParam("date", "2020/07/20 18:00")
			.formParam("pet", "George")
			.formParam("isPaid", "false")
			.formParam("_csrf", "${stoken}")
		).pause(10)
	}
	...
	val scn2 = scenario("Cannot create appointment for same day").exec(
		Home.home,
		Login2.login2,
		ShowHairdressers.showHairdressers,
		ShowHairdresser.showHairdresser,
		CannotAddAppointmentForSameDay.cannotAddAppointmentForSameDay
	)
